...
RUN apk add --no-cache musl-dev cmake make g++ curl-dev zlib-dev zlib-static perl protoc git
...
RUN cp target/$(if [ "$CARGO_BUILD_FLAGS" = "--release" ]; then echo "release"; else echo "debug"; fi)/rs-broker /app/rs-broker
